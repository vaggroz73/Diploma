# ================================================================
# LSTM PIPELINE ME SHAP ANALYSIS - ΕΝΙΑΙΟ ΑΡΧΕΙΟ
...
class Config:
    ...
    # LSTM Hyperparameters (βέλτιστες μετά από tuning)
    LSTM_UNITS_1    = 64
    NUM_LSTM_LAYERS = 1
    DROPOUT         = 0.4
    USE_BATCHNORM   = True
...
def build_model(input_shape, config):
    layers = [
        LSTM(config.LSTM_UNITS_1, return_sequences=False,
...
def run_shap_analysis(model, X_trainval, X_test, y_test, all_features, config):
    """Πλήρης SHAP analysis με GradientExplainer"""

    print("\n" + "="*60)
    print("SHAP ANALYSIS - Explainable AI για το LSTM μοντέλο")
    print("="*60)
    ...
    for t in range(config.SEQUENCE_LENGTH):
        ...
        shap.summary_plot(shap_values_t, X_test_t,
                          feature_names=all_features, max_display=20, show=False)
        plt.title(f'SHAP Feature Importance - {timestep_labels[t]}', fontsize=14)
        plt.tight_layout()
        plt.show()
    ...
    plt.title(f'Mean |SHAP| per Feature and Time Step (Top {top_n})', fontsize=14)
    plt.tight_layout()
    plt.show()
    ...
    if 'age' in all_features:
        ...
        plt.title(f'SHAP Dependence: {top_feature_name} (colored by age)', fontsize=13)
        plt.tight_layout()
        plt.show()
    ...
    if 'mid_season_transfer' in all_features:
        ...
        plt.title(f'SHAP Dependence: Mid-Season Transfer (colored by {top_feature_name})', fontsize=13)
        plt.tight_layout()
        plt.show()
    ...
    try:
        # No direct plotting for force_plot, so keep saving HTML or skip
        # For interactive display, it's typically rendered in a browser or specific notebook extensions
        # For this request, we'll keep the print statement and comment out the save if not needed
...
# ================================================================
print("="*60)
print("LSTM PIPELINE με SHAP Analysis - Walk-Forward Validation")
print("="*60)
...
model.save("lstm_phase_a.keras")
model_refit.save("lstm_phase_b_refit.keras")

pd.DataFrame({"id": val_ids, "y_true": y_val, "lstm_pred": val_pred}).to_csv(
    "lstm_val_predictions.csv", index=False)
pd.DataFrame({"id": test_ids, "y_true": y_test, "lstm_pred": test_pred}).to_csv(
    "lstm_test_predictions_seq_3.csv", index=False)
test_results.to_csv("test_predictions_with_transfer.csv", index=False)
feat_importance.to_csv("lstm_feature_importance_seq_3.csv", index=False)

print("  lstm_phase_a.keras")
print("  lstm_phase_b_refit.keras  <- ΤΕΛΙΚΟ ΜΟΝΤΕΛΟ")
print("  lstm_val_predictions.csv")
print("  lstm_test_predictions_seq_3.csv")
print("  test_predictions_with_transfer.csv")
print("  lstm_feature_importance_seq_3.csv")

# Plot training curves
plt.figure(figsize=(10, 6))
plt.plot(history.history['loss'], label='Train Loss')
plt.plot(history.history['val_loss'], label='Validation Loss')
plt.title('Training and Validation Loss Curves Over Epochs')
plt.xlabel('Epoch')
plt.ylabel('Loss (MSE)')
plt.legend()
plt.grid(True)
...
print("  ├── lstm_phase_a.keras")
print("  ├── lstm_phase_b_refit.keras (τελικό μοντέλο)")
print("  ├── lstm_val_predictions.csv")
print("  ├── lstm_test_predictions_seq_2.csv")
print("  ├── test_predictions_with_transfer.csv")
print("  ├── lstm_feature_importance_seq_2.csv")
