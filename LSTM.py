...
def run_shap_analysis(model, X_trainval, X_test, y_test, all_features, config):
    """Πλήρης SHAP analysis με GradientExplainer στο τελικό (tuned) μοντέλο."""
    ...
    # Επιλογή background data (από Train+Val)
   
    print("\n Προετοιμασία background data...")
...
# Preprocessor fit ΜΟΝΟ στο train 
...
# PHASE A  HYPERPARAMETER TUNING (Train → Val)
...
# Seed reset callback — κρίσιμο για reproducibility.
...
}).to_csv("lstm_val_predictions_1.csv", index=False)
print(f"  n = {len(val_pred_for_ensemble)} παίκτες")
print("  ok lstm_val_predictions.csv")



# PHASE B: FINAL MODEL REFIT (Train+Val → Test)

print("\n" + "="*60)
print("PHASE B: FINAL MODEL REFIT (Train+Val)")
print("="*60)
...
# SHAP analysis (explainability, στο final tuned model)
shap_values, shap_importance = run_shap_analysis(
    final_model, X_trainval, X_test, y_test, all_features_final, config
)



# ΣΥΝΟΨΗ TUNING TRIALS 
...
pd.DataFrame({
    ...
    "y_true":    y_test,
    "lstm_pred": test_pred,
}).to_csv("lstm_test_predictions_1.csv", index=False)
