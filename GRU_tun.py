# ================================================================
# GRU PIPELINE — ΜΕ HYPERPARAMETER TUNING (Keras Tuner)
# Διπλωματική Εργασία: ML στο Ποδόσφαιρο
# Target: Δoverall (Time Series Forecasting)
#
# ΣΤΡΑΤΗΓΙΚΗ (Walk-Forward Validation):
#   Phase A — Tuning  : preprocessor & sequences fit on TRAIN only
#                       tuner αναζητά βέλτιστες υπερπαραμέτρους
#                       με validation data = VAL set
#   Phase B — Refit   : ίδιος preprocessor (frozen, fit on TRAIN)
#                       final model εκπαιδεύεται σε TRAIN+VAL
#                       αξιολόγηση στο TEST (2023)
#
# PREPROCESSING POLICY:
#   Ο preprocessor γίνεται fit ΜΟΝΟ στο TRAIN set και παραμένει
#   frozen σε ολόκληρο το pipeline. Δεν γίνεται refit στο train+val
#   για αποφυγή data leakage (val statistics στον scaler).
#   Αυτό είναι η τυπική walk-forward προσέγγιση στη βιβλιογραφία.
#
# ΔΙΑΦΟΡΕΣ ΑΠΟ LSTM:
#   - GRU layers αντί LSTM (χωρίς cell state → λιγότερες παράμετροι)
#   - Ελαφρώς διαφορετικό search space στα units (GRU είναι πιο
#     αποδοτικό, οπότε δοκιμάζουμε και μεγαλύτερα μεγέθη)
#   - Tuner project name: "gru_delta_overall"
#   - Output files: gru_*.csv / gru_*.keras
# ================================================================

# ----------------------------------------------------------------
# Reproducibility — ΠΡΙΝ από οποιοδήποτε import TensorFlow/Keras
# ----------------------------------------------------------------
import os
import random
...
class Config:
    ...
    TUNER_PROJECT        = "gru_delta_overall"   # ← διαφορετικό από LSTM
...
# ================================================================
# COLUMN DEFINITIONS
# (Ίδιες ακριβώς με LSTM pipeline)
# ================================================================
FIFA_FEATURES = [
...
# ================================================================
# BUILD TUNABLE GRU MODEL
# ================================================================
def build_tunable_model(hp, input_shape):
    """
    GRU Search Space:
      gru_units_1    : 32 / 64 / 128 / 256
        (το 256 δεν υπήρχε στο LSTM — GRU έχει λιγότερες παράμετρους
         οπότε αξίζει να δοκιμαστεί μεγαλύτερο εύρος)
      gru_units_2    : 16 / 32 / 64 / 128
      num_gru_layers : 1 ή 2
    ...
    Σημείωση GRU vs LSTM:
      Η GRU layer δεν έχει recurrent_initializer ως ξεχωριστό
      kwarg στο Keras (χρησιμοποιεί ενιαίο kernel). Αντίστοιχα,
      το kernel_initializer καλύπτει και τα recurrent βάρη.
    """
    gru_units_1    = hp.Choice("gru_units_1",    values=[32, 64, 128, 256])
    gru_units_2    = hp.Choice("gru_units_2",    values=[16, 32, 64, 128])
    num_gru_layers = hp.Choice("num_gru_layers", values=[1, 2])
...
# ================================================================
print("="*60)
print("GRU PIPELINE — HYPERPARAMETER TUNING + WALK-FORWARD")
print("="*60)
...
# ================================================================
# PHASE A — STEP 10: HYPERPARAMETER TUNING
# ================================================================
print("\n" + "="*60)
...
print(f"  num_gru_layers : {best_hps.get('num_gru_layers')}")
...
pd.DataFrame({
    "id":       val_ids,
    "y_true":   y_val,
    "gru_pred": val_pred_for_ensemble,
}).to_csv("gru_val_predictions_1.csv", index=False)
print(f"  n = {len(val_pred_for_ensemble)} παίκτες")
print("  ok gru_val_predictions_1.csv")
...
final_model.save("gru_final_tuned.keras")
test_results.to_csv("gru_test_predictions_with_transfer.csv", index=False)
feat_importance.to_csv("gru_feature_importance.csv", index=False)
trials_df.to_csv("gru_tuning_results.csv", index=False)

pd.DataFrame({
    "id":       test_ids,
    "y_true":   y_test,
    "gru_pred": test_pred,
}).to_csv("gru_test_predictions_1.csv", index=False)

print("  gru_final_tuned.keras")
print("  gru_val_predictions_1.csv          <- tuning model (train-only) — για ensemble")
print("  gru_test_predictions_1.csv         <- final model (train+val)   — κύρια αξιολόγηση")
print("  gru_test_predictions_with_transfer.csv")
print("  gru_feature_importance.csv")
print("  gru_tuning_results.csv")

print("\n" + "="*60)
print("GRU PIPELINE COMPLETED SUCCESSFULLY!")
print("="*60)